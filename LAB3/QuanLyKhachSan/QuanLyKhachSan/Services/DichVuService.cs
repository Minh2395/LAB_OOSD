...
namespace QuanLyKhachSan.Services
{
    public class DichVuService
    {
        ...
        {
            return Db.Query(
                "SELECT d.SoPhieuDat, k.HoTen, c.SoPhong FROM PhieuDatPhong d " +
        ...
        {
            return Db.Query(
                "SELECT p.SoPhieuSDDV, p.SoPhong, p.NgaySuDung, d.TenDV, " +
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    var st = new SqlCommand(
                        "SELECT TrangThai FROM PhieuDatPhong " +
                        "WHERE SoPhieuDat = @s",
                        cn,
                        tx);
                    ...
                    var g = new SqlCommand(
                        "SELECT DonGia FROM DichVu WHERE MaDV = @d",
                        cn,
                        tx);
                    ...
                    var f = new SqlCommand(
                        "SELECT SoPhieuSDDV FROM PhieuSuDungDV " +
                    ...
                    var chk = new SqlCommand(
                        "SELECT COUNT(*) FROM ChiTietPhieuSuDungDV " +
                        "WHERE SoPhieuSDDV = @so AND MaDV = @d",
                        cn,
                        tx);
                    ...
                    return KetQuaXuLy.Ok(
                        "Đã ghi nhận dịch vụ.Nếu cùng dịch vụ được dùng nhiều lần trong ngày, " +
