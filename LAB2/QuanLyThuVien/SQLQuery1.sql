...
-- Quy tắc được khóa ở CSDL
-- TheDocGia có unique filtered index để mỗi độc giả chỉ có một thẻ TrangThai=1
-- ChiTietPhieuMuon có UNIQUE(MaPhieuMuon, MaDauSach) để không trùng đầu sách trong một phiếu
-- SoLuongHienCo có CHECK >= 0
-- => Các ràng buộc này bổ sung lớp bảo vệ bên dưới Business Logic.