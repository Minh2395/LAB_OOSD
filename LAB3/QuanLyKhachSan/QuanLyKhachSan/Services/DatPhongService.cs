...
namespace QuanLyKhachSan.Services
{
    public class DatPhongService
    {
        ...
        {
            return Db.Query(
                "SELECT * FROM NguoiLuuTru WHERE SoPhieuDat = @s " +
                ...
                try
                {
                    foreach (var x in ds)
                    {
                        var q = new SqlCommand(
                            "SELECT SoNguoiToiDa FROM Phong WHERE SoPhong = @p",
                            cn,
                            tx);
                        ...
                        if (x.SoNguoi <= 0 || x.SoNguoi > Convert.ToInt32(o))
                            return KetQuaXuLy.Fail(
                                "Số người của phòng " + x.SoPhong + " vượt sức chứa.");

                        if (PhongTrungLich(cn, tx, x.SoPhong, nhan, tra))
                            return KetQuaXuLy.Fail(
                                "Phòng " + x.SoPhong + " bị trùng lịch đặt.");
                    }

                    var h = new SqlCommand(
                        "INSERT INTO PhieuDatPhong" +
                        "(SoPhieuDat, MaKhach, MaNVLeTan, NgayLap, NgayNhan, " +
                    ...
                    foreach (var x in ds)
                    {
                        var c = new SqlCommand(
                            "INSERT INTO ChiTietDatPhong VALUES(@s, @p, @n)",
                            cn,
                            tx);
                        ...
                        var u = new SqlCommand(
                            "UPDATE Phong SET TrangThai=N'Đã đặt' WHERE SoPhong = @p",
                            cn,
                            tx);
            ...
            string qt)
        {
            ...
            try
            {
                int max = Convert.ToInt32(
                    Db.Scalar(
                        "SELECT SoNguoi FROM ChiTietDatPhong " +
                        "WHERE SoPhieuDat = @s AND SoPhong = @p",
                        new SqlParameter("@s", so),
                        new SqlParameter("@p", phong)));

                int dem = Convert.ToInt32(
                    Db.Scalar(
                        "SELECT COUNT(*) FROM NguoiLuuTru " +
                        "WHERE SoPhieuDat = @s AND SoPhong = @p",
                        new SqlParameter("@s", so),
                        new SqlParameter("@p", phong)));
        ...
        public KetQuaXuLy NhanPhong(string so, DateTime thucTe)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                ...
                {
                    var c = new SqlCommand(
                        "UPDATE PhieuDatPhong SET TrangThai=N'Đang ở'," +
                        "NgayNhanThucTe=@n " +
        ...
        public KetQuaXuLy DanhDauNoShow(string so)
        {
            try
            {
                ...
                return KetQuaXuLy.Ok(
                    "Đã đánh dấu không nhận phòng. Việc mất cọc " +
                    "được giữ theo quy định gốc; đề không nêu thời gian tự động " +
                    "nên thao tác này do nhân viên xác nhận.");
            }
            catch (Exception ex)
            {
                return KetQuaXuLy.Fail(ex.Message);
