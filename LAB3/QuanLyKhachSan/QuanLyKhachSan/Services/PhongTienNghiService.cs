...
namespace QuanLyKhachSan.Services
{
    public class PhongTienNghiService
    {
        ...
        {
            return Db.Query(
                "SELECT t.*,l.TenLoaiTN FROM TienNghi t JOIN " +
                "LoaiTienNghi l ON t.MaLoaiTN = l.MaLoaiTN ORDER BY t.MaTienNghi");
        }

        public DataTable LayLapDat()
        {
            return Db.Query(
                "SELECT p.*,l.TenLoaiTN FROM PhieuLapDat p JOIN " +
                "TienNghi t ON p.MaTienNghi = t.MaTienNghi JOIN LoaiTienNghi l ON " +
                "t.MaLoaiTN = l.MaLoaiTN ORDER BY NgayLap DESC");
        }
        ...
        public KetQuaXuLy ThemPhong(string so, string khu, int max, decimal gia)
        {
            ...
            try
            {
                Db.Execute(
                    "INSERT INTO " +
                    "Phong(SoPhong, MaKhuVuc, SoNguoiToiDa, DonGiaNgay, TrangThai) " +
            ...
            string ghiChu)
        {
            ...
            try
            {
                ...
                Db.Execute(
                    "UPDATE TienNghi SET TinhTrangHienTai=@tt " +
                    "WHERE MaTienNghi = @m",
                    new SqlParameter("@tt", tinhTrang),
