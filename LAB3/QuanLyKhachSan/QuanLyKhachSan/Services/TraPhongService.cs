...
namespace QuanLyKhachSan.Services
{
    public class TraPhongService
    {
        ...
        {
            return Db.Query(
                "SELECT h.*,k.HoTen FROM HoaDon h " +
            ...
            int soNgay)
        {
            ...
            try
            {
                ...
                return KetQuaXuLy.Ok(
                    ...
                    "Số ngày tính tiền là dữ liệu nhân viên xác nhận vì đề gốc " +
                    "không quy định cách làm tròn ngày lưu trú.");
            }
            catch (Exception ex)
            {
                return KetQuaXuLy.Fail(ex.Message);
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    var q = new SqlCommand(
                        "SELECT TongTien FROM HoaDon WHERE SoHoaDon = @h",
                        cn,
                        tx);
                    ...
                    var paid = new SqlCommand(
                        ...
                        "FROM ThanhToan WHERE SoHoaDon = @h",
                        cn,
                        tx);
                    ...
                    return KetQuaXuLy.Ok(
                        "Đã ghi nhận thanh toán bằng " + hinhThuc + ".");
                }
                catch (Exception ex)
                {
                    try
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                ...
                {
                    var q = new SqlCommand(
                        "SELECT h.SoHoaDon,h.TrangThai FROM HoaDon h " +
                        "WHERE h.SoPhieuDat = @s",
                        cn,
                        tx);
